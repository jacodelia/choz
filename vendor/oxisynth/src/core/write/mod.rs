...
impl Core {
    ...
    /// Every audio group's next sample, into `out` (as many as fit), with
    /// the reverb and chorus returns mixed into group 0 as [`Self::read_next`]
    /// does. choz patch — see CHOZ-PATCH.md.
    #[inline]
    pub fn read_next_groups(&mut self, out: &mut [(f32, f32)]) {
        if self.output.cur == 64 {
            self.one_block(false);
            self.output.cur = 0;
        }
        let cur = self.output.cur;
        for (g, slot) in out.iter_mut().enumerate().take(self.output.left_buf.len()) {
            *slot = (self.output.left_buf[g][cur], self.output.right_buf[g][cur]);
        }
        self.output.cur += 1;
    }

    /// How many audio groups the synth renders. choz patch.
    pub fn audio_groups(&self) -> usize {
        self.output.left_buf.len()
    }
