...
impl Synth {
    ...
    /// Every audio group's next sample — a group is the MIDI channels
    /// `channel % audio_groups` — with reverb and chorus in group 0. choz
    /// patch: upstream renders the groups and only ever reads the first.
    #[inline(always)]
    pub fn read_next_groups(&mut self, out: &mut [(f32, f32)]) {
        self.core.read_next_groups(out)
    }

    /// How many audio groups this synth renders. choz patch.
    pub fn audio_groups(&self) -> usize {
        self.core.audio_groups()
    }
