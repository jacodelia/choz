...
fn main() {
    ...
    for (fmt, path, id) in [
        ...
        (
            PluginFormat::Vst3,
            "/home/jorge/.vst3/u-he/TyrellN6.vst3",
            "",
        ),
        (
            PluginFormat::Vst2,
            "/home/jorge/.vst/u-he/TyrellN6.64.so",
            "",
        ),
    ] {
        if !std::path::Path::new(path).exists() {
            println!("{path}: not here");
            continue;
        }
        match choz_engine::engine::build_instrument(
            fmt,
