...
impl SidechainDuck {
    ...
    pub fn set_release(&mut self, secs: f32) {
        self.release_secs = secs.clamp(0.001, 4.0);
    }
    pub fn set_hold(&mut self, secs: f32) {
        self.hold_secs = secs.clamp(0.0, 1.0);
    }
    pub fn set_rate(&mut self, hz: f32) {
        self.rate_hz = hz.clamp(0.0, 20.0);
    }
...
mod tests {
    ...
    fn no_effect_with_zero_depth() {
        ...
        for (a, b) in input.iter().zip(buf.iter()) {
            assert!(
                (a - b).abs() < 1e-6,
                "zero depth should pass through unchanged"
            );
        }
    }
}
