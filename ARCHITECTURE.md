...
├── src/
│   ├── main.rs             # App state, event loop, UI rendering, mouse/keyboard input
│   ├── engine.rs           # Real-time audio engine (cpal callback thread)
│   ├── fx_chain.rs         # Builds FX processor chains from specification structs
│   ├── source.rs           # Audio source types, FX kind enum, parameter descriptors
│   ├── registry.rs         # Plugin registry — unified lifecycle for all plugin formats
│   ├── scanner.rs          # Filesystem discovery for LADSPA/DSSI/SFZ/SF2/JSFX plugins
│   ├── plugin_types.rs     # Plugin format enum, descriptor struct, host port trait
│   ├── views/
...
│   └── fx/
│       ├── mod.rs          # FxProcessor trait + FxParam descriptor
